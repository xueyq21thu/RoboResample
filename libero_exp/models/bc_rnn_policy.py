...
class BCRNNPolicy(BasePolicy):
    ...
    def __init__(self, cfg, shape_meta):
        ...
        self.language_encoder = eval(policy_cfg.language_encoder.network)(      # 0.024608 M
            **policy_cfg.language_encoder.network_kwargs
        )
        rnn_input_size += text_embed_size

        ### 3. encode extra information (e.g. gripper, joint_state)
        self.extra_encoder = ExtraModalities(       # 0 M
        ...
        self.rnn_projector = nn.Linear(rnn_input_size, policy_cfg.rnn_hidden_size)  # 0.17408 M     129->1024
        self.rnn = nn.LSTM(     # 16.7936 M
            input_size=policy_cfg.rnn_hidden_size,
        ...
        self.policy_head = eval(policy_cfg.policy_head.network)(    # 2.106375 M
    ...
    def forward(self, data, train_mode=True, return_latent=False):
        # 1. encode image
        encoded = []
        ...
        encoded = self.rnn_projector(encoded)
        if train_mode:
            h0 = torch.zeros(
