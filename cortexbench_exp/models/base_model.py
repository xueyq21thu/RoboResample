...
class BaseModel(nn.Module, metaclass=ModelMeta):
    ...
    def grad_forward(self, data):
        cfg= self.cfg
        images, extra_states = data
        embedding = self.get_representations(images.to(self.device))
        # embedding = self.vector_extractor(embedding)
        # return embedding
    
        extra = None
        if cfg.env.add_proprio:
            extra = self.extra_states_encoder(extra_states.reshape(1, -1).to(self.device))
        ...
        if cfg.policy.use_spatial:
            ...
            if len(self.latent_queue_proprio) > self.history_window:
                self.latent_queue_proprio.pop(0)
            extra = torch.cat(self.latent_queue_proprio, dim=0).unsqueeze(0)    # (1, t, emb_dim)
