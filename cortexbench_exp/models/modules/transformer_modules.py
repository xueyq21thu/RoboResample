...
from .image_encoder import PatchEncoder
...
class TransformerEncoder(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        super().__init__()
        self.patch_encoder = PatchEncoder(input_shape=(3, 224, 244))  

        # patch_pos_embed = nn.Parameter(torch.randn(1, 49, mlp_hidden_size))  
        # self.register_parameter("patch_pos_embed", patch_pos_embed)

        spatial_token = nn.Parameter(torch.randn(1, 1, mlp_hidden_size))  # SPATIAL_TOKEN
    ...
    def forward(self, x, mask=None):
        ...
        x = rearrange(x, "b c h w -> b (h w) c")    # (B, 64, 7, 7) -> (B, 49, 64)
        # x += self.patch_pos_embed.unsqueeze(0)  # (B, 49, E)

        spatial_token = self.spatial_token.expand(x.shape[0], -1, -1)  # (B, 1, E) = (B, 1, 128)
        x = torch.cat([spatial_token, x], -2)
...
class SmallViT(nn.Module):
    def __init__(self, image_size=224, patch_size=16, dim=256, depth=4, heads=4, mlp_dim=512, dropout=0.1):
        """
        A smaller ViT-based model with reduced parameters.
    ...
    def forward(self, x):
        ...
        patches = patches.permute(0, 2, 3, 1, 4, 5).reshape(B, -1, 3 * 16 * 16)
