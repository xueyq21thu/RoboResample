...
class BC_IB_Policy(BaseAlgo):
    def __init__(self, cfg, inference=False):
        ...
        if not inference:
            if cfg.policy.use_spatial:
                input_dim = (cfg.policy.embedding_dim + cfg.policy.extra_states_encoder.input_size) * cfg.env.history_window
            else:
                mul = 1
                if cfg.env.add_proprio:
                    mul += 1
                input_dim = cfg.policy.spatial_down_sample.input_size * (mul+1) * cfg.env.history_window
            hidden_dim = 512
    ...
    def compute_loss(self, data):
        ...
        bc_loss = self.model.policy_head.loss_fn(dist, data["actions"].to(self.device), reduction="mean")
