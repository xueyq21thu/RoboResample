...
def load_r3m(modelid, load_path=None):
    ...
    if load_path == None:
        ...
        modelpath = os.path.join(load_path, "model.pt")
        configpath = os.path.join(load_path, "config.yaml")
...
def load_r3m_reproduce(modelid, load_path=None):
    home = os.path.join(expanduser("~"), ".r3m")
    if modelid == "r3m":
